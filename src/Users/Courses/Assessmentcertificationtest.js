    ...
    <div className="" >
      <div className="row status">
        <div className='question_status_info'>
          ...
          <h5 className="mock-test1">Section</h5>
          <h6>Logical Reasoning</h6>
        </div>
        {questions.map((_, index) => (
          <div className="col-6 col-md-3" key={index} style={{ marginBottom: '10px' }}>
            <button
...
function Assessmentcertificationtest() {
  ...
  useEffect(() => {
    ...
    const fetchCardData = async () => {
      try {
        ...
        setQuestioncount(response.data.assessment.length)      
      } catch (e) {
...
const questions = assessmentdata && assessmentdata.map((each) => ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      await axios.post(`${apiList.submitAssignmentforcertification}/${learningPathTitle}`,{userId:userdata,Questioncount:questioncount,userAnswers : userAnswers,userdetails:userdetails} );
      ...
      setTimeout(() => {
        navigate("/Assessmentcertificationresult",{state:{Timelimit:Timelimit,timeLeft:timeLeft,CategoryTitle:learningPathTitle,userId:userdata,Modalname:Modalname}})
      }, 1000);
    } catch (error) {
      console.error(error);
    }
...
// code
export default Assessmentcertificationtest;
