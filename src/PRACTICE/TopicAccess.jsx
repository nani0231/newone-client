...
const TopicAccess = () => {
	...
	const { TopicName,topicId,CategoryId } = state || {};
	...
	const handleCheckboxChange2 = (rowId, batchyearId, batchId) => {
		...
		const AddAccessGiven = {
			PracticeAccessTopic: Access,
		};
		console.log(AddAccessGiven);
		axios
			.post(
				`${apiList.AccessGivenByTopic}/${rowId}/${batchyearId}/${batchId}/${CategoryId}/${topicId}`,
				AddAccessGiven,
				{
					headers: {
						token: token,
	...
	// const handleCheckboxChange2 = (rowId, batchyearId, batchId) => {
	//   // Toggle the state of the checkbox locally
	//   setSelectedRows((prevSelectedRows) => {
	//     if (prevSelectedRows.includes(batchId)) {
	//       return prevSelectedRows.filter((id) => id !== batchId);
	//     } else {
	...
	//   const updatedCategoriesAccess = {
	//     CategoriesAccess: selectedRows.includes(batchId) ? "On" : "Off",
	//   };

	//   // Make a POST request to update CategoriesAccess in the backend
	//   axios
	//     .post(
	//       `${apiList.UpdateCategoriesAccess}/${rowId}/${batchyearId}/${batchId}`,
	//       updatedCategoriesAccess,
	//       {
	//         headers: {
	//           token: token,
	//         },
	...
	//       setdata1(response.data);
	//       if (response.status === 200) {
	//         toast("CategoriesAccess Updated Successfully", {
	//           position: "top-center",
	//           autoClose: 1000,
	//           hideProgressBar: false,
	//           closeOnClick: true,
	...
	//     .catch((error) => {
	//       if (error.response && error.response.status === 400) {
	//         toast("Error updating CategoriesAccess", {
	//           position: "top-center",
	//           autoClose: 1000,
	//           hideProgressBar: false,
	//           closeOnClick: true,
	...
	//       } else {
	//         console.log(error.message);
	//         setError("An error occurred while updating CategoriesAccess.");
	//       }
	//     });
	// };
	...
	// const selectedRows2 =()=>{const ischecked = addblogslist.filter(v => v.Access === "On").map(v=>v._id);
	// const selectedRows2 =()=>{const ischecked = addblogslist.map(v => v.InstituteBatchYear).map(x=>x.InsituteBatch).filter(z=>z.LearningPathAccess === "on").map(z=>z._id)
	const selectedRows2 = () => {
		const ischecked = addblogslist.flatMap((blog) =>
			blog.InstituteBatchYear.flatMap((batchYear) =>
				batchYear.InsituteBatch.filter(
					(batch) => batch.PracticeCategoriesTopicAccess.some(
						(each) => each.PracticeTopicAccess === "On" && each.categoryId === CategoryId && each.topicId ===topicId
					  )
				).map((batch) => batch._id)
			)
		);
		console.log(ischecked, "sriram");
										...
										<div className="card-item p-4">
											<div className="row">
												<div className="col-md-12 text-center">
													<h4 className="" style={{color: "rgb(22, 195, 234)"}}>{TopicName} Access</h4>
												</div>
