...
const CategoriesAccess = () => {
	...
	const handleCheckboxChange2 = (rowId, batchyearId, batchId) => {
		...
		const AddAccessGiven = {
			AccessCategory: Access,
		};
		console.log(AddAccessGiven);
		axios
			.post(
				`${apiList.AccessGivenByCategory}/${rowId}/${batchyearId}/${batchId}/${CategoryId}`,
				AddAccessGiven,
				{
					headers: {
						token: token,
	...
	const selectedRows2 = () => {
		const ischecked = addblogslist.flatMap((blog) =>
		  blog.InstituteBatchYear.flatMap((batchYear) =>
			batchYear.InsituteBatch.filter((batch) =>
			  batch.PracticeCategoriesAccess.some(
				(each) => each.AccessCategory === "On" && each.CategoryId === CategoryId
			  )
			).map((batch) => batch._id)
		  )
		);
										...
										<div className="card-item p-4">
											<div className="row">
												<div className="col-md-12 text-center">
													<h4 className="" style={{color: "rgb(22, 195, 234)"}}>{CategoryTitle} Categories Access</h4>
												</div>
